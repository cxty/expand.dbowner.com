...
class loginMod extends commonMod{
	...
	public function setLoginInfo(){	
		...
		if($rbArr){
			//是否是插件开发者，若是普通开发者，则不能进入平台
			if ( $rbArr['uLevel'] === 0 ) {
				$this->redirect('/throwMessage/throwMsg?tt=2&msgkey=Ex_NoUsePluginValid');
			}
			...
			$this->redirect('/throwMessage/throwMsg?tt=2&msgkey=Ex_NotPluginDevelopers');
		}
		...
		if ( strpos($url, '/throwMessage') !== false ) {
			$url = '/plugIn';
		} 
		*/
		$url = '/plugIn';
